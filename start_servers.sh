...
# Stack = Qwen2.5-72B-AWQ LLM (:8001) + Whisper STT (:8002). TTS is OFF-box (Uplift AI),
# so there is no local TTS server here.
# Usage:  bash /workspace/Build3/start_servers.sh
cd /workspace

echo "Starting LLM (Qwen2.5-72B-AWQ) on :8001 ..."
tmux kill-session -t llm 2>/dev/null
tmux new -s llm -d 'vllm serve /workspace/models/qwen72 --served-model-name sara-llm --max-model-len 4096 --gpu-memory-utilization 0.78 --enable-auto-tool-choice --tool-call-parser hermes --port 8001 2>&1 | tee /workspace/llm.log'
